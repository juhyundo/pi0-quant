...
Usage
-----
    python rmse_exp/automate_rel_sweep.py \\
        --impl ipt_numba \\
...
def _build_quant_server_template(
    *,
    ...
    {rel_err} and {quantized_port} are left as literal placeholders.
    """
    parts = [
        python, str(serve_script),
        "--config",         config,
...
def _find_resume_dir(output_dir: Path, impl: str) -> Optional[Path]:
    runs = sorted(output_dir.glob(f"run-*-{impl}"))
...
def main() -> None:
    ...
    from rmse_exp.impl_registry import require_rel_err
    impl = require_rel_err(args.impl)
    serve_script = impl["serve_quant"]
    fp8_mode     = impl["default_fp8_mode"]
    ...
    impl_tag = args.impl
    if args.resume:
        run_dir = _find_resume_dir(output_root, impl_tag)
        if run_dir is None:
            logger.warning("--resume specified but no existing run found; starting fresh")
            run_dir = output_root / _timestamp_tag(impl_tag)
        else:
    ...
    logger.info("Sweep: %d format combinations  impl=%s", len(combos), impl_tag)
    run_log_fh.write(f"# run_dir={run_dir}\n")
    run_log_fh.write(f"# impl={impl_tag}  combos={len(combos)}  n_obs={args.n_obs}  seed={args.seed}\n")
    run_log_fh.write(f"# rel_err_step={args.rel_err_step}  rmse_threshold={args.rmse_threshold}\n\n")
    run_log_fh.flush()
...
def _timestamp_tag(impl: str) -> str:
    return time.strftime(f"run-%Y%m%d-%H%M%S-{impl}", time.localtime())


def _parse_args() -> argparse.Namespace:
    from rmse_exp.impl_registry import IMPLS
    p = argparse.ArgumentParser(
        description=(
            "Sweep 16 (input_fmt x output_fmt) quantization combos with rel_err noise, "
            "recording RMSE vs. rel_err and generating summary plots."
        )
    )
    p.add_argument("--impl", default="ipt_numba", choices=list(IMPLS),
                   help="Implementation to benchmark (default: ipt_numba)")
    ...
    p.add_argument("--only-combos", default=None,
                   help=(
                       "Comma-separated INPUT:OUTPUT pairs to run. "
    ...
    _SHORT_TO_FULL.update({k: k for k in _SHORT})
    if ns.only_combos:
        resolved = []
